...
const userWithPosts = query(async (ctx, a: { id: string }) =>
...
describe("Runtime", () => {
  ...
  test("a mutation commits and reports a WAL position; a query cannot write", async () => {
    await withApp(async (sql0, n) => {
      ...
      const err = await rt.runQuery(sneaky, {}).then(
        () => null,
        (e: unknown) => e,
      );
  ...
  test("a serialization conflict is retried and both mutations land", async () => {
    await withApp(async (sql0, n) => {
      await sql0.unsafe(
        `create table dzb_app.counters(id int primary key, n int not null); insert into dzb_app.counters values (1, 0)`,
      );
      const rt = new Runtime({ sql: sql0, schema, publication: n.publication });
  ...
  test("a statement issued after the handler returned never runs: the connection is closed to it", async () => {
    await withApp(async (sql0, n) => {
      ...
      const outcome = await (late as unknown as Promise<unknown>).then(
        () => "ran",
        (e: unknown) => String((e as { cause?: Error })?.cause?.message ?? e),
      );
  ...
  test("two db.transaction() blocks started together both land (savepoints are serialized)", async () => {
    await withApp(async (sql0, n) => {
      ...
      const both = mutation(async (ctx) => {
        const r = await Promise.allSettled([
          ...
          ctx.db.transaction(async (tx) => {
            await tx.insert(posts).values({ authorId: U, title: "a" });
            return "a";
          }),
          ctx.db.transaction(async (tx) => {
            await tx.insert(posts).values({ authorId: U, title: "b" });
            await Bun.sleep(30);
            return "b";
          }),
        ]);
      ...
      const titles = (await sql0.unsafe("select title from dzb_app.posts order by title")).map(
  ...
  test("functions that change session state are refused (a search_path set here would outlive the transaction)", async () => {
    await withApp(async (sql0, n) => {
      ...
      for (const text of ["select set_config('search_path', 'dzb_app', false)", "select pg_advisory_lock(42)"]) {
        ...
        const err = await rt.runQuery(q, {}).then(
          () => null,
          (e: unknown) => e,
        );
  ...
  test("a serialization failure raised AT COMMIT (write skew) is retried, and the invariant holds", async () => {
    await withApp(async (sql0, n) => {
      await sql0.unsafe(
        `create table dzb_app.doctors(id int primary key, on_call boolean not null); insert into dzb_app.doctors values (1, true), (2, true)`,
      );
      const rt = new Runtime({ sql: sql0, schema, publication: n.publication });
      let arrived = 0;
      let release!: () => void;
      ...
      const goOff = mutation(async (ctx, a: { id: number }) => {
        const [row] = await ctx.db.execute<{ c: number }>(
          sql`select count(*)::int as c from dzb_app.doctors where on_call`,
        );

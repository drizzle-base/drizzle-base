...
describe("buildReadSet", () => {
	test("tables resolve to schema.name, quoted or not", async () => {
		await withApp(async (sql, n) => {
			...
			const r = await rs(sql, c, `select * from dzb_app.users u where not exists (select 1 from "dzb_app"."posts" p where p.author_id = u.id)`);
	...
	test("non-immutable functions and SQL value functions are volatile; immutable ones are not", async () => {
		await withApp(async (sql, n) => {
			const c = new Catalog(n.publication);
			expect((await rs(sql, c, `select now(), random() from dzb_app.users`)).volatile.sort()).toEqual(["now", "random"]);
	...
	test("a partition leaf also names its root; the root names its leaves", async () => {
		await withApp(async (sql, n) => {
			const c = new Catalog(n.publication);
			expect(sorted(await rs(sql, c, `select * from dzb_app.events_1`))).toEqual(["dzb_app.events", "dzb_app.events_1"]);
	...
	test("an inheritance parent also names its children", async () => {
		await withApp(async (sql, n) => {
			expect(sorted(await rs(sql, new Catalog(n.publication), `select * from dzb_app.animals`))).toEqual(["dzb_app.animals", "dzb_app.dogs"]);
		});
	});
...
describe("shapes the final review found escaping the read-set", () => {
	test("built-in functions that run SQL, and user operators, make it opaque", async () => {
		await withApp(async (sql, n) => {
			const c = new Catalog(n.publication);
			expect((await rs(sql, c, `select query_to_xml('select * from dzb_app.posts', true, false, '')`)).opaque.join()).toMatch(/query_to_xml reads tables/);
			expect((await rs(sql, c, `select table_to_xml('dzb_app.posts', true, false, '')`)).opaque.join()).toMatch(/table_to_xml reads tables/);
	...
	test("every table the scan reaches must be in the publication, not just one of the family", async () => {
		await withApp(async (sql, n) => {
			const onlyParent = `pub_only_parent_${process.pid}`, onlyChild = `pub_only_child_${process.pid}`;
			await sql.unsafe(`create publication ${onlyParent} for table only dzb_app.animals; create publication ${onlyChild} for table dzb_app.dogs`);
			try {
				...
				expect((await rs(sql, new Catalog(onlyChild), `select * from dzb_app.animals`)).opaque.join()).toMatch(/dzb_app.animals is not in the publication/);
			} finally {

		...
		for (const spec of specifiers(f.text)) {
			if (spec.startsWith(".")) {
				...
				if (!target.startsWith("src/")) {
					...
					if (from !== null) problems.push(`${f.path}: imports ${target}, outside src/ (production code may not depend on tests or benches)`);
					continue;
				}
			...
			if (spec === "drizzlebase" || spec.startsWith("drizzlebase/")) {
				...
				if (from !== null) problems.push(`${f.path}: imports the package's own public entry ${spec}; import the module instead`);
				continue;
			}

...
import { dropCapture, ensureCapture, PgoutputCapture } from "../../src/capture";
...
await sql.unsafe(
  `create schema "${n.schema}"; create table "${n.schema}".t(id bigint primary key, v int); create table "${n.schema}".noise(id bigserial primary key, v int, pad text)`,
);
...
await cap.start({
  onEvent: (e) => {
    if (e.kind === "txn")
      for (const c of e.txn.changes) {
        const id = Number(c.new?.["id"]);
        waiters.get(id)?.(performance.now());
      }
  },
  onError: (e) => {
    console.error(e);

...
export class Catalog {
	...
	private async loadRelation(exec: SQL, name: string, only: boolean): Promise<RelationInfo | null> {
		...
		if (!row) return null;
		return { schema: row.schema, name: row.name, relkind: row.relkind, members: row.members as Member[], ancestors: row.ancestors as string[] };
	}
	...
	private async loadFunction(exec: SQL, f: FunctionRef): Promise<FunctionInfo | null> {
		...
		return { user: rows.some((r: { ns: string }) => r.ns !== "pg_catalog"), volatile: rows.some((r: { vol: string }) => r.vol !== "i") };
	}

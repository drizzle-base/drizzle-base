...
export class PgoutputCapture {
	...
	private open(h: CaptureHandlers): Promise<void> {
		...
		const service = new LogicalReplicationService({ ...this.opts.connection, keepAlive: true }, {
			...
			service.on("start", () => {
				...
				if (adv) {
					...
					this.advancer = setInterval(() => {
						void emitBarrier(adv.sql, `drizzlebase.advance.${++n}`).catch((e) => log.warn("advance barrier failed", { error: String(e) }));
					}, adv.everyMs);
				}
				resolve();
			});

  ...
  unsafe(query: string, params: unknown[] = []): ReturnType<ReservedSQL["unsafe"]> {
    ...
    if (this.mode === "query" && parsed.kind !== "select")
      throw new ForbiddenStatementError(
        `${parsed.kind.toUpperCase()} in a query: queries are read-only, use a mutation`,
      );
    for (const f of collectRefs(parsed.stmt).functions)
      if (SESSION_STATE.has(f.name))
        throw new ForbiddenStatementError(
          `${f.name}() changes session state, which would outlive this function's transaction`,
        );

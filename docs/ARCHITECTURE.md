...
| `server` | the WebSocket handler (`createHandler`), the API registry (`defineApi`), error redaction, `startDrizzleBase` (boot + capture restart) (01a-4a) | the server modules + protocol |
| `protocol` | wire frames, the value codec (Date/bigint/bytes), client-frame validation; no Node APIs (01a-4a) | — |
